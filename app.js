...
const localLat = 40.5697;
const localLon = -74.6099;
        ...
        content: [
            {
                ...
                content: [
                    ...
                    { type: 'component', componentName: 'localForecast', title: '7-DAY GEOGRAPHIC SYNOPTIC OUTLOOK (08876)' }
                ]
            },
            {
                type: 'column',
                width: 30,
                content: [
                    { type: 'component', componentName: 'nwsAlerts', title: 'CRITICAL ENVIRONMENTAL SPECTRUM HAZARDS MATRIX - NEW JERSEY' },
                    { type: 'component', componentName: 'hydrologyFeed', title: 'DELAWARE & RARITAN CANAL REAL-TIME STREAMFLOW' }
                ]
            },
            {
                type: 'column',
                ...
                content: [
                    ...
                    { type: 'component', componentName: 'noaaTides', title: 'NOAA TIDES & CURRENTS (STATION 8531463 - NEW BRUNSWICK)' }
                ]
            }
        ]
    }]
...
layout.registerComponent('radarMap', function(container) {
    container.getElement().html(`
        <div style="position:relative; width:100%; height:100%; background:#0d1117;">
            ...
            <iframe id="windyIframe" src="https://embed.windy.com/embed.html?type=map&location=coordinates&metricRain=in&metricTemp=f&metricWind=mph&zoom=8&overlay=radar&product=radar&level=surface&lat=40.5697&lon=-74.6099" style="width:100%; height:100%; border:none;"></iframe>
        </div>
    `);
    ...
    setTimeout(() => {
        ...
        select.on('change', function() {
            ...
            iframe.src = `https://embed.windy.com/embed.html?type=map&location=coordinates&metricRain=in&metricTemp=f&metricWind=mph&zoom=8&overlay=${layer}&product=${product}&level=surface&lat=40.5697&lon=-74.6099`;
        });
    }, 200);
});
...
layout.registerComponent('cloudMap', function(container) {
    container.getElement().html(`
        <div style="position:relative; width:100%; height:100%; background:#0d1117;">
            ...
            <iframe id="windyCloudIframe" src="https://embed.windy.com/embed.html?type=map&location=coordinates&metricRain=in&metricTemp=f&metricWind=mph&zoom=8&overlay=clouds&product=gfs&level=surface&lat=40.5697&lon=-74.6099" style="width:100%; height:100%; border:none;"></iframe>
        </div>
    `);
...
layout.registerComponent('nwsAlerts', function(container) {
    ...
    container.on('open', fetchNewJerseyAlerts);
});
...
layout.registerComponent('hydrologyFeed', function(container) {
    ...
    container.on('open', fetchSomersetHydrology);
});
...
function fetchNewJerseyAlerts() {
    const container = $('#alerts-container');
    console.log("Starting New Jersey alerts fetch...");
    
    // Use the NWS alerts API with a broad New Jersey search
    // Query all alerts and filter for NJ
    fetch('https://api.weather.gov/alerts/active')
        ...
        .then(data => {
            ...
            // Filter for New Jersey alerts only
            let njAlerts = allAlerts.filter(f => {
                const areaDesc = f.properties.areaDesc || '';
                const properties = f.properties;
                
                // Check if alert applies to NJ
                return areaDesc.includes('NJ') || 
                       areaDesc.includes('New Jersey') ||
                       (properties.areaDesc && properties.areaDesc.toLowerCase().includes('new jersey'));
            });
            
            console.log("New Jersey alerts found:", njAlerts.length);
            njAlerts.forEach(alert => {
            ...
            if (njAlerts.length > 0) {
                html += `<div style="margin-bottom:10px; padding-bottom:8px; border-bottom:1px solid #30363d;">
                    <div style="font-size:0.75rem; color:#ffcc00; font-weight:bold; margin-bottom:5px;">ACTIVE ALERTS - ${njAlerts.length}</div>`;
                ...
                html = "<span style='color:#00ff55; font-size:0.8rem;'>✓ SYSTEM CLEAN: NO ACTIVE ALERTS FOR NEW JERSEY</span>";
            }
        ...
        .catch(err => {
            console.error("New Jersey alerts fetch error:", err);
...
function openAlertDetails(id) {
    ...
    openFloatingModal(`NEW JERSEY ALERT DETAILS`, body);
}
            ...
            let html = `
                <div style="margin-bottom:8px; padding-bottom:6px; border-bottom:1px dashed #30363d;">
                    <div style="font-size:0.85rem; color:#fff;">
                        <i class="fa-solid fa-satellite-dish"></i> SOMERVILLE / SOMERSET COUNTY, NJ (08876)
...
function fetchNOAATides() {
    // NOTE: Somerset County is inland; its nearest tidal water is the Raritan
    // River at New Brunswick, just downstream of where the D&R Canal empties
    // in. 8531463 (New Brunswick, Raritan River, NJ) is the closest tide
    // station but is a PREDICTION-ONLY subordinate station referenced to
    // Sandy Hook - it has no real-time water level or meteorological sensors.
    // 8531680 (Sandy Hook, NJ) is the nearest active NOAA NWLON station with
    // live water level and air temperature data and is used for the
    // "observed" readings below.
...
function fetchSomersetHydrology() {
...
setInterval(() => {
    ...
    if(countdownVal <= 0) {
        countdownVal = 120;
        fetchNWSForecast();
        fetchNewJerseyAlerts();
        fetchAirQualityData();
        fetchNOAATides();
        fetchSomersetHydrology();
    }
